...
export default function RevenueShare({
    ...
    const isFormValid = (): boolean => {
        ...
        if (!state.createThresholdStructures) {
            return state.sharePercentage > 0;
        }

        return state.structures.every(structure => structure.tiers[0]?.sharePercentage > 0);
    };
    useEffect(() => {
        if (!isEditable) {
            if (isDirty) {
                                ...
                                !state.createThresholdStructures && (
                                    <div className="space-y-2">
                                        ...
                                        {state.sharePercentage <= 0 && (
                                            <p className="text-sm text-red-500">Share percentage must be greater than 0</p>
                                        )}
                                    </div>
                                )}
                                                        ...
                                                        <div key={tierIndex} className="space-y-2 p-4 border rounded mt-4">
                                                            ...
                                                            <div className="space-y-2">
                                                                ...
                                                                <NumericFormat
                                                                    ...
                                                                    disabled={!isEditable}
                                                                />
                                                                {tierIndex === 0 && tier.sharePercentage <= 0 && (
                                                                    <p className="text-sm text-red-500">Share percentage must be greater than 0</p>
                                                                )}
                                                            </div>
