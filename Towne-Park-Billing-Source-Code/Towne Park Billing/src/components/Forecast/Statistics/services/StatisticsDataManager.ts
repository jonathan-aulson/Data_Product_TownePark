...
export class StatisticsDataManager {
    ...
    static async saveStatistics(
        ...
        timePeriod: TimeRangeType,
        calculateOvernightSelfForMonth: (monthIndex: number, periodKey: string) => number,
        calculateOvernightValetForMonth: (monthIndex: number, periodKey: string) => number,
        calculateExternalRevenueForMonth: (monthIndex: number, periodKey: string) => number,
        customers: Customer[]
    ): Promise<void> {
        ...
        for (let monthIndex = 0; monthIndex < 3; monthIndex++) {
            ...
            const monthForecastArray: SiteStatisticDetailData[] = Object.entries(monthForecastValues).map(([periodKey, values]) => {
                ...
                return {
                    ...
                    selfOvernight: calculateOvernightSelfForMonth(monthIndex, periodKey),
                    valetOvernight: calculateOvernightValetForMonth(monthIndex, periodKey),
                    ...
                    externalRevenue: calculateExternalRevenueForMonth(monthIndex, periodKey)
                };
            });
