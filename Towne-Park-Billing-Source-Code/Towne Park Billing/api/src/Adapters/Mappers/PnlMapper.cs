...
namespace api.Adapters.Mappers
{
    ...
    public static partial class PnlMapper
    {
        ...
        public static PnlResponseDto PnlVoListToDto(List<(int Year, PnlBySiteListVo Vo)> allPnlVos)
        {
            ...
            response.BudgetRows = propertyNames.Select(prop => new PnlRowDto
            {
                ...
                MonthlyValues = pnlVo.Budget
                    ...
                    .Select(g =>
                    {
                        ...
                        // Populate site breakdown for columns needed by calculators
            ...
            response.VarianceRows = propertyNames.Select(prop =>
            {
                ...
                {
                    var budgetVal = pnlVo.Budget.Where(x => getMonth(x) == month).Sum(x => GetValue(x, prop));
                    var actualVal = pnlVo.Actual.Where(x => getMonth(x) == month).Sum(x => GetValue(x, prop));
                    var varianceAmount = actualVal - budgetVal;
                    decimal? variancePercent = budgetVal != 0 ? (varianceAmount / budgetVal) * 100 : (actualVal != 0 ? 100 : 0);
                ...
                decimal? totalVariancePercent = totalBudget != 0 ? (totalVarianceAmount / totalBudget) * 100 : (totalActual != 0 ? 100 : 0);
                return new PnlVarianceRowDto
                {
                    ColumnName = prop,
