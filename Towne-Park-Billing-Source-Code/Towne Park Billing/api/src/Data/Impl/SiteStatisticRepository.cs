...
namespace api.Data.Impl
{
    public class SiteStatisticRepository : ISiteStatisticRepository
    {
        ...
        public async Task<List<SiteStatisticDetailVo>> GetBudgetData(string siteId, string billingPeriod)
        {
        ...
        public async Task<List<SiteStatisticDetailVo>> GetBudgetDataForRange(string siteId, List<string> billingPeriods)
        {
            // If only one period, use the existing method
            if (billingPeriods.Count == 1)
            {
                return await GetBudgetData(siteId, billingPeriods[0]);
            }

            // For multiple periods, make parallel requests and combine results
            var tasks = billingPeriods.Select(period => GetBudgetData(siteId, period)).ToList();
