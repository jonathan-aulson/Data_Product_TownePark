...
namespace api.Data.Impl
{
    public class BillingStatementRepository : IBillingStatementRepository
    {
        ...
        private void AddIsCurrentConditionToQuery(QueryExpression query)
        {
            ...
            // Condition 2: bs_BillingStatement.Fields.CreatedOn matches the current month and year
            var currentDate = DateTime.Now;
            var startOfMonth = new DateTime(currentDate.Year, currentDate.Month, 1);
            var endOfMonth = startOfMonth.AddMonths(1).AddDays(-1).AddHours(23).AddMinutes(59).AddSeconds(59); // Last day of the current month

            var dateCondition = new ConditionExpression(bs_BillingStatement.Fields.CreatedOn,
                ConditionOperator.Between, new object[] { startOfMonth, endOfMonth });

            // Creating filter to combine conditions with OR logic
            var filter = new FilterExpression(LogicalOperator.Or);
            filter.AddCondition(statusCondition);
            filter.AddCondition(dateCondition);
